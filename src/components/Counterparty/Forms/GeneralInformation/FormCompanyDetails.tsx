...
export const FormCompanyDetails: React.FC<Props> = ({setChangeCompanyDetails}) => {
    ...
    const {id, full_name, short_name, group, branches,parent_id}: any = AuthorData;
    const classes = useStyles();
    const [branch, setBranches] = useState(`${branches.length > 0 ? branches[0].id : ''}`)
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            changeAuthorCompanyDetailsData({
                'full_name': values.FullCompanyName, 'short_name': values.ShortNameCompany,
                'parent_id': parent_id, 'branches': [branch,branch1,branch2].filter(tiem => tiem.length > 0)
            }, id,errorMessage)
        ...
        <div className={classes.root}>
            <form onSubmit={formik.handleSubmit}>
                ...
                <Paper className={classes.paper}>
                    {errorMsg == 'CompanyDetails' && <div style={{color: "red"}}>{error}</div>}
            ...
            <InputFilterSelectedBranches
                name="Industry"
                handleChange={(e: React.ChangeEvent<HTMLInputElement>) => setBranches(e.target.value)}

            />
              </div>
                          {branch1 && <div style={{marginBottom: 15,display:"flex"}}>
                            <InputFilterSelectedBranches
                                name="Industry"
                                handleChange={(e: React.ChangeEvent<HTMLInputElement>) => setBranches1(e.target.value)}
                          ...
                          {branch2 && <div style={{marginBottom: 15,display:"flex"}}>
                            <InputFilterSelectedBranches
                                name="Industry"
                                handleChange={(e: React.ChangeEvent<HTMLInputElement>) => setBranches2(e.target.value)}
