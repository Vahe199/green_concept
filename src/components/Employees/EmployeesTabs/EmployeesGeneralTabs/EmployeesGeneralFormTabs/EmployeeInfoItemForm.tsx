...
const EmployeeInfoItemForm: React.FC<EmployeeFormDataProps> = ({
  setEmployeeData,
}) => {
  const { updateEmployeeEmployeeDataAC, recoveryEmployeesState } = useActions();
  const { error, success_update, loading, employeeById } = useTypedSelector(
          ...
          Object.entries(values).forEach((item) => {
            ...
            if (Array.isArray(value)) {
              value.forEach((val, index) => {
                formData.append(
                  `${key}[][${nameMapper[key as keyof typeof nameMapper]}]`,
                  val
                );
              });
            } else {
